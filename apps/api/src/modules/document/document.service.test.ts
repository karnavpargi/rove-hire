...
describe('DocumentService', () => {
  ...
  describe('validateOfferInput (via generateOfferDocuments)', () => {
    ...
    it('should reject missing role title', async () => {
      ...
      await expect(service.generateOfferDocuments(input, 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('generateOfferDocuments - prerequisite checks', () => {
    ...
    it('should reject when candidate not found', async () => {
      mockPrisma.candidate.findUnique.mockResolvedValue(null);
      await expect(service.generateOfferDocuments(validInput, 'user-1')).rejects.toThrow(
        'Candidate not found',
      );
    });
    ...
    it('should reject when candidate has no completed interview', async () => {
      mockPrisma.candidate.findUnique.mockResolvedValue({
        ...
        interviews: [{ status: 'Scheduled', feedback: null }],
      });
      await expect(service.generateOfferDocuments(validInput, 'user-1')).rejects.toThrow(
        'at least one completed interview with feedback',
      );
    });
    ...
    it('should reject when interview is completed but has no feedback', async () => {
      mockPrisma.candidate.findUnique.mockResolvedValue({
        ...
        name: 'Test Candidate',
        status: 'InterviewScheduled',
        interviews: [{ status: 'Completed', feedback: '' }],
      });
      await expect(service.generateOfferDocuments(validInput, 'user-1')).rejects.toThrow(
        'at least one completed interview with feedback',
      );
  ...
  describe('getDocumentUrl', () => {
    it('should return presigned URL for existing document', async () => {
      ...
      expect(mockFileService.getPresignedUrl).toHaveBeenCalledWith(
        'documents/offer-letters/test.pdf',
      );
    });
  ...
  describe('findAll', () => {
    it('should return all documents with candidate info ordered by createdAt desc', async () => {
      const mockDocuments = [
        {
          id: 'doc-1',
          ...
          candidate: { id: 'candidate-1', name: 'Alice Johnson' },
        },
      ];
      mockPrisma.document.findMany.mockResolvedValue(mockDocuments);
