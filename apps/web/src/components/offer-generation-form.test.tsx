...
describe('OfferGenerationForm', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders submit and cancel buttons', () => {
      ...
      expect(
        screen.getByRole('button', { name: /generate offer documents/i }),
      ).toBeInTheDocument();
      expect(
  ...
  describe('Client-side Validation', () => {
    ...
    it('shows error when salary has more than 2 decimal places', () => {
      ...
      expect(
        screen.getByText(/at most 2 decimal places/i),
      ).toBeInTheDocument();
    });
    ...
    it('shows error when start date is in the past', () => {
      ...
      expect(
        screen.getByText(/start date must be today or in the future/i),
      ).toBeInTheDocument();
    });
    ...
    it('shows error when reporting manager is empty on blur', () => {
      ...
      expect(
        screen.getByText('Reporting manager is required'),
      ).toBeInTheDocument();
    });
    ...
    it('clears error when valid input is provided after blur', () => {
      ...
      expect(
        screen.queryByText('Role title is required'),
  ...
  describe('Cancel button', () => {
    it('calls onCancel when cancel button is clicked', () => {
      ...
      renderWithProviders(
        <OfferGenerationForm {...defaultProps} onCancel={onCancel} />,
      );
  ...
  describe('Form submission', () => {
    ...
    it('shows success state with download links on successful generation', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/offer documents generated/i),
        ).toBeInTheDocument();
      });

      // Verify download links
      expect(
        screen.getByRole('link', { name: /download offer letter/i }),
      ).toHaveAttribute('href', 'https://s3.example.com/offer.pdf');
      expect(
        screen.getByRole('link', { name: /download nda/i }),
      ).toHaveAttribute('href', 'https://s3.example.com/nda.pdf');
    });
  });
});
